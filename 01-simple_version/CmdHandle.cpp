...
CmdHandle::State CmdHandle::SendList(bool detail) {
...
CmdHandle::Connection CmdHandle::GetConnect() { /* 得到一个连接 */
	...
	if (passive_) { /* 如果是被动模式 */
		...
		CMD cmd = kExpectConn;
		Write(commufd_, &cmd, sizeof(cmd));
		unsigned short port = ntohs(clnaddr_.sin_port);
		Write(commufd_, &port, sizeof(port)); /* 传递端口号 */
		char* ip = inet_ntoa(clnaddr_.sin_addr);
		size_t len = strlen(ip);
		printf("ip is %s\n", ip);
		Write(commufd_, &len, sizeof(len)); /* 首先传递长度 */
		Write(commufd_, ip, len); /* 传递ip地址 */

		Read(commufd_, &cmd, sizeof(cmd));
		if (cmd == kGotFd) {
			datafd = recvfd(commufd_);
		}
		return boost::make_shared<Conn>(datafd);
	}
}
